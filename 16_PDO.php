...
<body>
    ...
    try {
        $pdo = new PDO(
            'mysql:host=localhost;dbname=société',  // driver mysql + nom du serveur de la BDD + nom de la BDD 
    ...
    */
    ...
    $result = $pdo->query("SELECT * FROM employes WHERE prenom = 'louca' ");
    ...
    $result = $pdo->query("SELECT * FROM employes WHERE prenom = 'hugo' ");
    $employe = $result->fetch();  // transforme l'objet $result en un ARRAY associatif et numérique
    debug($employe);
    echo $employe['prenom'] . '<br>';
    echo $employe[1] . '<br>';

    $result = $pdo->query("SELECT * FROM employes WHERE prenom = 'kassidi' ");
